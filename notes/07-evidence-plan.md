...
### 06·07 가로 넘침
콘솔에서 아래를 실행하고 **결과값이 함께 보이게** 찍습니다. `[1366, 768, 0]`처럼
크기와 넘침이 한 줄에 나옵니다.

```js
[innerWidth, innerHeight, document.documentElement.scrollWidth - document.documentElement.clientWidth]
```

값 하나만 찍으면 **몇 픽셀에서의 `0`인지 증명되지 않습니다.** 크기를 같은 줄에 넣는 이유입니다.

창 크기는 **개발자 도구의 반응형 모드**(`Ctrl`+`Shift`+`M`)로 맞춥니다. 모니터 크기와
무관하게 뷰포트가 정확히 그 값이 되고, 배율을 `Fit`으로 두면 화면도 잘리지 않습니다.

> 이전 판에는 "반응형 모드는 스크롤바 폭이 달라지니 실제 창 크기로 맞추라"고
> 적혀 있었습니다. **이 페이지에는 해당하지 않습니다.** 재측정에서 두 크기 모두
> `clientWidth`가 `innerWidth`와 같았습니다 — 자리를 차지하는 스크롤바가 아예
> 없어서 두 방식의 측정값이 같습니다.
...
캡처 원본 17장은 **`실행 이미지/`**에 있습니다. `evidence/`와 마찬가지로
`.gitignore`에 들어 있어 저장소에 올라가지 않습니다.

| # | 항목 | 상태 |
|---|---|---|
| 17 | 개인정보·비밀값 0건 | **완료** `evidence/E17-secret-scan.txt` |
| 18 | 빌드·검사 통과 | **완료** `evidence/E18-build-test-lint.txt` |
| 01~05 | 실행 화면 5종 | **완료** 6장. E03·E04는 재도전 버튼 배포 직전 빌드라 결과 화면 버튼이 하나다 |
| 06·07 | 두 해상도 넘침 | **부분** — 뷰포트 크기(`1366 × 768`·`1920 × 1080`)는 찍혔으나 콘솔 값이 빠졌다. 넘침 0 자체는 별도 실측으로 확인됨 |
| 08·09 | 음소거·움직임 줄이기 | **완료** 3장. `E09-motion-before`는 별도 파일이 없지만, `E08-mute-before`가 같은 설정창에서 `REDUCE MOTION // OFF`를 보여주므로 **그 장이 직전 상태를 겸한다** |
| 10 | 포커스 이탈 일시정지 | 미촬영 — 감사 판정은 실측으로 이미 통과라 필수는 아니다 |
| 11 | 저장 손상 복구 | **완료** 1장(잘못된 JSON). 13종 전부는 아니고, 새로고침 뒤 화면이라 심은 값 자체는 콘솔에 남지 않았다. 복구 동작은 공개본에서 확인됨 |
| 12·13 | 콘솔·네트워크 | **완료.** E13은 `Domain` 열을 켜서 5건 전부 `myeongjundev.github.io`인 것이 보인다 |
| 14 | 10분 연속 실행 | **미달** — 두 장의 파일 시각이 15:23 → 15:26으로 약 3분이다. 게다가 힙 수치가 `Take snapshot` 결과가 아니라 프로파일 종류 선택 화면의 실시간 값이라 **GC 전 숫자**다 |
| ~~15·16~~ | ~~난이도 실험~~ | **취소** |

### 14를 다시 찍는다면

메모리 패널에서 **왼쪽 위 ⏺ `Take snapshot`을 실제로 눌러야** 합니다. 그래야
GC가 돌고 두 시점을 비교할 수 있습니다. 지금 두 장은 그 버튼을 누르기 전,
프로파일 종류를 고르는 화면입니다.

한 판이 30초이므로 **연속 20판이 곧 10분**입니다.
