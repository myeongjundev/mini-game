...
| 인트로·로비 | 구현됨, **코드 6건 해결 / 이미지 2건 남음, 미푸시** |
| 테스트 | 92개 통과 |
...
이미지 두 건은 **직접 하셔야 합니다.** 학원 PC에도 PIL·ImageMagick·sharp가
없는 것을 확인했습니다. 이미지를 만든 도구에서 모니터 화면을 비우고
1180px 이하 WebP로 다시 뽑으세요. 셸 최대 폭이 1180px이라 그보다 큰 원본은
어차피 쓰이지 않습니다.

M-5만 따로 급하면 `npm i -D sharp` 후 변환 스크립트로 처리할 수 있습니다.
모니터 화면 비우기(m-3)는 코드로 덮으면 티가 나서 원본 도구가 필요합니다.
