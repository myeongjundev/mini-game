...
> **2026-08-20 갱신 — 한 가지가 뒤집혔다.**
> 이 문서는 "모달을 CRT 화면 안쪽에 두고 벗어나지 않게" 하라고 지시하고,
> 처음에는 그렇게 구현했다. **플레이해보니 본문이 118px밖에 안 나와 가이드가
> 계속 잘렸다.** 모달을 씬 전체로 옮겨 1366에서 900×443이 됐고 다섯 쪽이
> 스크롤 없이 들어온다. 그림 속 모니터는 로비 화면의 무대이지 대화상자의
> 경계가 아니라고 봤다. 나머지 지시(공용 프레임, 글자는 HTML, 비율 335:165,
> pixelated, 금지 사항)는 그대로 지킨다. 근거와 실측은
> `docs/QA_CHECKLIST.md` 12절에 있다.
